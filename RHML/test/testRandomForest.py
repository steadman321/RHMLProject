...
def simpleClassTest():
    ...
    myRandomForest = ClassificationRandomForest.RHMLClassificationRandomForest(2,5,X_train,y_train,**{"min_split":25,"max_depth":4,"include_OOB":True})

    print(f"OOB Score : {myRandomForest.OOBScore}")
    
    predictions = myRandomForest.predict(X_test)
    ...
    featureImp = myRandomForest.calcFeatureImportance()  
    plotFeatureImportance([featureImp])

    proximityMatrix = myRandomForest.calcProximity(X_train)
...
def simpleRegressionTest():
    ...
    myRandomForest = RegressionRandomForest.RHMLRegressionRandomForest(2,5,X_train,y_train,**{"min_split":25,"max_depth":4})
    ...
    myRandomForest.calcFeatureImportance()  
    proximityMatrix = myRandomForest.calcProximity(X_train)
